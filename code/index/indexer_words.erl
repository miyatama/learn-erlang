...
-export([words_in_file/3, 
     process_word/2]).

-define(DEBUG(S), io:fwrite("[DEBUG] indexer_words: " ++ S ++ "~n")).
-define(DEBUG(S, Args), io:fwrite("[DEBUG] indexer_words: " ++ S ++ "~n", Args)).

words_in_file(Pid, File, EtsTrigrams) ->
    ?DEBUG("words_in_file/3"),
...
to_lower_case([H|T]) when $A=< H, H=<$Z -> 
    [H+$a-$A|to_lower_case(T)];
to_lower_case([H|T]) -> 
    [H|to_lower_case(T)];
to_lower_case([]) -> 
    [].
